...
class LedgerSafetyInstrumentedTest {
    ...
    fun moneyOutUsesOcrCounterpartyWithoutUploaderIdentity() {
        ...
        composeRule.onNodeWithTag("receipt_to_value")
            .assertTextContains("Event ledger")
        ...
        composeRule.onNodeWithTag("receipt_from_value")
            .assertTextContains("Event ledger")
    ...
    fun labelledAmountCanSaveWhenOptionalDetailsAreMissing() {
        ...
        composeRule.onNodeWithTag("receipt_amount_value")
            .assertTextContains("₹54,000.00")
        composeRule.onNodeWithTag("receipt_ledger_person_input")
            .assertTextContains("Organizer")
        composeRule.onNodeWithTag("receipt_to_value")
            .assertTextContains("Event ledger")
        assertTrue(composeRule.onAllNodesWithTag("receipt_amount_input").fetchSemanticsNodes().isEmpty())
        assertTrue(composeRule.onAllNodesWithTag("receipt_amount_confirmation").fetchSemanticsNodes().isEmpty())
        assertTrue(composeRule.onAllNodesWithText("Extracted JSON").fetchSemanticsNodes().isEmpty())
        assertTrue(composeRule.onAllNodesWithText("Confidence:", substring = true).fetchSemanticsNodes().isEmpty())
        assertTrue(composeRule.onAllNodesWithText("Optional details do not block", substring = true).fetchSemanticsNodes().isEmpty())
    ...
    @Test
    fun unlabelledAmountCannotBeManuallyOverridden() {
        ...
        assertTrue(composeRule.onAllNodesWithTag("receipt_amount_confirmation").fetchSemanticsNodes().isEmpty())
        assertTrue(
            composeRule.onAllNodesWithText("Scan a clearer receipt with a labelled amount.", substring = true)
                .fetchSemanticsNodes()
                .isNotEmpty()
        )
    ...
    @Test
    fun reliableDetectedAmountIsReadOnlyAndSavesUnchanged() {
        val (viewModel, _) = launchEventDetails()
        val sharedReceiptText = """
            Google Pay
            Paid ₹65,653
        ...
        assertEquals(65653.0, viewModel.selectedEventTransactions.value.single().amount, 0.0)
    }
