...
class EventDaoIntegrityInstrumentedTest {
    ...
    @Test
    fun transactionInsertRejectsProspectiveAggregateOverflow() = runBlocking {
        val eventId = eventDao.insertEvent(
            EventEntity(title = "Overflow Guard", customFieldsJson = "{}")
        ).toInt()
        val first = eventDao.insertTransaction(
            TransactionEntity(
                eventId = eventId,
                personName = "First",
                personPhone = "",
        ...
        val rejected = eventDao.insertTransaction(
            TransactionEntity(
                eventId = eventId,
                personName = "Second",
                personPhone = "",
        ...
        assertEquals(1, eventDao.getTransactionsForEventOnce(eventId).size)
    }

    @Test
    fun unlinkedTransactionsUseStableSourceOrderForRepair() = runBlocking {
        val eventId = eventDao.insertEvent(
            EventEntity(title = "Repair Ordering", customFieldsJson = "{}")
        ).toInt()
        eventDao.insertTransaction(
            TransactionEntity(
                id = 20,
        ...
        assertEquals(listOf(10, 20), eventDao.getUnlinkedTransactionsForEvent(eventId).map { it.id })
    }
}