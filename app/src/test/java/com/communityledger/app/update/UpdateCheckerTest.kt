package com.communityledger.app.update
...
class UpdateCheckerTest {
    ...
    fun `unpublished channel is reported without a download`() {
        val result = UpdateChecker.evaluateManifest(
            jsonText = """{"schemaVersion":1,"applicationId":"${BuildConfig.APPLICATION_ID}","available":false}""",
    ...
    fun `lookalike GitHub host is rejected`() {
        val result = UpdateChecker.evaluateManifest(
            newerManifest().replace("github.com", "github.com.communityledger.app.org"),
            currentVersionCode = BuildConfig.VERSION_CODE
        )
    ...
    @Test
    fun `different application id is rejected`() {
        val result = UpdateChecker.evaluateManifest(
            newerManifest().replace(BuildConfig.APPLICATION_ID, "com.aistudio.communityledger.yrtqwx"),
            currentVersionCode = BuildConfig.VERSION_CODE
        )

        assertEquals(UpdateCheckResult.Failed("Release information belongs to a different app."), result)
    }
