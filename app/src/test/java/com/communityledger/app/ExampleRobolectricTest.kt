...
import com.communityledger.app.ui.Screen
...
class ExampleRobolectricTest {
    ...
    private fun awaitDeepLinkIdentityRequest(viewModel: EventViewModel, timeoutMillis: Long = 30_000L) {
        val deadline = System.nanoTime() + timeoutMillis * 1_000_000L
        while (!viewModel.deepLinkIdentityRequired.value && System.nanoTime() < deadline) {
            shadowOf(Looper.getMainLooper()).idle()
            Thread.sleep(25)
        }
        shadowOf(Looper.getMainLooper()).idle()
    ...
    fun `read string from context`() {
        ...
        assertEquals("Community Ledger Dev", appName)
    }
    ...
    @Test
    fun `event copy link requires local identity before mutation`() {
        val app = ApplicationProvider.getApplicationContext<Application>()
        val preferences = app.getSharedPreferences("app_prefs", Context.MODE_PRIVATE)
        ...
        try {
            ...
            val creator = "creator@gmail.com"
            val signature = viewModel.generateInviteChecksum(eventId, expiry, creator)
            val encodedEventId = viewModel.encodeEventId(eventId)
            val deepLinkUri = Uri.parse(
                "https://gopib9119.github.io/event/join?eventId=$encodedEventId" +
                    "&expiry=$expiry&checksum=$signature&creatorEmail=$creator&title=IdentityRequired"
            )

            viewModel.handleDeepLink(deepLinkUri)
            awaitDeepLinkIdentityRequest(viewModel)

            assertTrue(viewModel.deepLinkIdentityRequired.value)
            assertEquals(listOf(Screen.Dashboard), viewModel.navigationStack)
            assertNull(viewModel.deepLinkMessage.value)

            assertTrue(viewModel.setMyUserEmail("recipient@example.com"))
            assertTrue(viewModel.resumePendingEventCopy())
            awaitDeepLinkResult(viewModel)

            assertFalse(viewModel.deepLinkIdentityRequired.value)
            assertEquals(
                "Added an independent copy of 'IdentityRequired'. You were not added as a member; entries and balances do not sync.",
                viewModel.deepLinkMessage.value
            )
            assertTrue(viewModel.navigationStack.last() is Screen.EventDetails)
        } finally {
            preferences.edit().remove("user_email").commit()
        }
    }
    ...
    @Test
    fun `local ledger person mutation requires local identity`() {
        val app = ApplicationProvider.getApplicationContext<Application>()
        val preferences = app.getSharedPreferences("app_prefs", Context.MODE_PRIVATE)
        preferences.edit().remove("user_email").commit()
    ...
    fun `legacy title change cannot create another event shell`() {
        val app = ApplicationProvider.getApplicationContext<Application>()
        val viewModel = EventViewModel(app)
        viewModel.setMyUserEmail("local.user@example.com")
