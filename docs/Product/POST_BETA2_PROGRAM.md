...
- Improve image guidance, amount emphasis, evidence source, missing-field explanation, Money in/out selection, ledger-person attribution, confirmation, duplicate, cancel, retry, and interruption states.
...
- Every saved amount comes from user input or real ML Kit output and is explicitly reviewed.
- App-private JSON is written before the Room mutation; failure leaves totals unchanged.
- Labelled, unlabelled, edited, duplicate, non-receipt, missing-amount, process-death, and database-failure paths pass focused checks.
...
| Receipt review | Amount evidence and ledger impact clear; duplicate/missing/edit confirmation gates; compact responsive UI |
...
.\gradlew.bat --no-daemon --no-configuration-cache :app:connectedDebugAndroidTest
